...
type (
	...
	// Aggregator holds information about the payment aggregator.
	Aggregator struct {
		// SubMerchantId is the sub-merchant ID.
		// [Optional]
		SubMerchantId string `json:"sub_merchant_id,omitempty"`
		// AggregatorIdVisa is the Visa identifier for the payment aggregator.
		// [Optional]
		AggregatorIdVisa string `json:"aggregator_id_visa,omitempty"`
		// AggregatorIdMc is the Mastercard identifier for the payment aggregator.
		// [Optional]
		AggregatorIdMc string `json:"aggregator_id_mc,omitempty"`
	}

	// PaymentProcessing represents the processing object returned by POST /payments
	// (PaymentResponse.processing). It is a distinct schema from ProcessingData,
	// which is returned by GET /payments/{id}.
	PaymentProcessing struct {
		// RetrievalReferenceNumber is a unique identifier for the authorization that is
		// submitted to the card scheme during processing.
		// [Optional]
		RetrievalReferenceNumber string `json:"retrieval_reference_number,omitempty"`
		// AcquirerTransactionId is a unique identifier for the transaction generated by the acquirer.
		// [Optional]
		AcquirerTransactionId string `json:"acquirer_transaction_id,omitempty"`
		// RecommendationCode is a code representing the follow-up action to take with the
		// payment, as recommended by Checkout.com.
		// [Optional]
		RecommendationCode string `json:"recommendation_code,omitempty"`
		// Scheme is the scheme the transaction was processed with.
		// [Optional]
		Scheme string `json:"scheme,omitempty"`
		// PartnerMerchantAdviceCode is the Merchant Advice Code (MAC) provided by Mastercard,
		// which contains additional information about the transaction. For declined
		// transactions it also indicates whether the payment can be retried and how long to wait.
		// [Optional]
		PartnerMerchantAdviceCode string `json:"partner_merchant_advice_code,omitempty"`
		// PartnerResponseCode is the original authorization response code sent by the scheme,
		// also referred to as the raw response code. It differs from RecommendationCode, which
		// is provided by Checkout.com and is standardized across all schemes.
		// [Optional]
		PartnerResponseCode string `json:"partner_response_code,omitempty"`
		// PartnerOrderId is the unique order identification of an Afterpay payment.
		// [Optional]
		PartnerOrderId string `json:"partner_order_id,omitempty"`
		// PartnerPaymentId is the unique identification of a payment provided by partner.
		// [Optional]
		PartnerPaymentId string `json:"partner_payment_id,omitempty"`
		// PartnerStatus is the status of a payment provided by partner.
		// [Optional]
		PartnerStatus string `json:"partner_status,omitempty"`
		// PartnerTransactionId is the unique transaction identification provided by partner.
		// [Optional]
		PartnerTransactionId string `json:"partner_transaction_id,omitempty"`
		// PartnerSessionId is the partner session identifier associated with the payment.
		...
		// PartnerErrorCodes are the error codes provided by partner.
		// [Optional]
		PartnerErrorCodes []string `json:"partner_error_codes,omitempty"`
		// PartnerErrorMessage is the error description provided by partner.
		// [Optional]
		PartnerErrorMessage string `json:"partner_error_message,omitempty"`
		// PartnerAuthorizationCode is the authorization code provided by partner.
		// [Optional]
		PartnerAuthorizationCode string `json:"partner_authorization_code,omitempty"`
		// PartnerAuthorizationResponseCode is the authorization response code provided by partner.
		// [Optional]
		PartnerAuthorizationResponseCode string `json:"partner_authorization_response_code,omitempty"`
		// SurchargeAmount is the surcharge amount applied to the transaction in minor units.
		// [Optional]
		// >= 0
		SurchargeAmount int64 `json:"surcharge_amount,omitempty"`
		// PanTypeProcessed is the type of Primary Account Number (PAN) used for the payment.
		...
		// FallbackSourceUsed indicates whether the fallback_source field was used for the payment.
		// [Optional]
		FallbackSourceUsed bool `json:"fallback_source_used,omitempty"`
		// CkoNetworkTokenAvailable is the flag indicating if a Checkout Network Token was
		// available for the payment.
		// [Optional]
		CkoNetworkTokenAvailable bool `json:"cko_network_token_available"`
		// PurchaseCountry is the country where the purchase was made, as an ISO 3166-1
		...
		// SchemeMerchantId is the merchant identifier that was configured with the scheme
		...
		// Aggregator holds the aggregator information for this payment.
		// [Optional]
		Aggregator *Aggregator `json:"aggregator,omitempty"`
		// PartnerClientToken is the client token returned by the partner for client-side
		// initialization.
		// Not documented in the public spec for this response, kept for backward compatibility.
		// [Optional]
		PartnerClientToken string `json:"partner_client_token,omitempty"`
		// ContinuationPayload is a payload used to continue processing for multi-step payment flows.
		// Not documented in the public spec for this response, kept for backward compatibility.
		// [Optional]
		ContinuationPayload string `json:"continuation_payload,omitempty"`
		// Pun is the Payment Unique Number (PUN) assigned to the payment.
		// Not documented in the public spec for this response, kept for backward compatibility.
		// [Optional]
		Pun string `json:"pun,omitempty"`
		// MerchantCategoryCode is the merchant category code (MCC) associated with the payment.
		// Not documented in the public spec for this response, kept for backward compatibility.
		// [Optional]
		MerchantCategoryCode string `json:"merchant_category_code,omitempty"`
		// SchemeTransactionLinkId is the scheme transaction link identifier. Returned for
		// Mastercard transactions when the scheme provides a link identifier that ties
		// together related transactions on the network.
		// [Optional]
		SchemeTransactionLinkId string `json:"scheme_transaction_link_id,omitempty"`
	}
