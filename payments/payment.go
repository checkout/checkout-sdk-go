...
type (
    ...
    // Marketplace - Information related to marketplace payments
    Marketplace struct {
        SubEntityID string       `json:"sub_entity_id,omitempty"`
        SubEntities []*SubEntity `json:"sub_entities,omitempty"`
    }

    // SubEntity - The sub-entity that the payment is being processed on behalf of.
    SubEntity struct {
        ID         string      `json:"id"`
        Amount     uint64      `json:"amount"`
        Reference  string      `json:"reference,omitempty"`
